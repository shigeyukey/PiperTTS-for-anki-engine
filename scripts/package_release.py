...
def write_release_notes(dist_directory: str, tag: str, engine_version: str, repository: str, file_entries, builds):
    lines = []
    lines.append(f"# piper engines {engine_version}")
    lines.append("")
    lines.append("Engine files for the PiperTTS V2 Anki add-on, built by GitHub Actions.")
    lines.append("The add-on downloads this release at runtime; nothing is bundled with the add-on.")
    lines.append("")
    ...
    lines.append("## How the add-on uses this release")
    lines.append("")
    lines.append(f"- release repository: `{repository}`")
    lines.append(f"- release tag: `{tag}`")
    lines.append("- download files: `manifest.json`, then the zip named in `manifest.json`")
    lines.append("- the add-on verifies the zip size and sha256 before installing it")
    lines.append("")
    lines.append("## Licenses")
    lines.append("")
    lines.append("The engine is built from the sources below; their license texts are included in `licenses/`.")
    lines.append("")
    lines.append("- espeak-ng: GPL-3.0 (https://github.com/espeak-ng/espeak-ng)")
    lines.append("- libpiper (piper1-gpl): GPL-3.0 (https://github.com/OHF-Voice/piper1-gpl)")
    lines.append("- jpreprocess: BSD-3-Clause (https://github.com/jpreprocess/jpreprocess)")
    lines.append("- onnxruntime-web: MIT (see `downloaded/LICENSE.txt`)")
    lines.append("")
    lines.append("This file is generated by `scripts/package_release.py`.")
    lines.append("")
...
def main() -> int:
    ...
    notes_path = write_release_notes(dist_directory, arguments.tag, engine_version, arguments.repository, file_entries, builds)
